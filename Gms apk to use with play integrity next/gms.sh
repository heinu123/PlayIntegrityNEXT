...
su -c 'if [ -f /data/adb/pif.json ]; then
  rm "/data/adb/pif.json" && echo "File /data/adb/pif.json removed."
else
  echo "Pif.json not present, let's continue"
fi' >> /storage/emulated/0/gms_termux.log

su -c 'if pgrep -f com.google.android.gms > /dev/null; then
...
su -c 'if pgrep -f com.google.android.gms.unstable > /dev/null; then
